...
OverviewPage::OverviewPage(String name, LiquidCrystal *display, Button *btn,
                           Encoder *enc, VolumeTracker *volumeTrackers[],
                           bool areInput[])
    ...
    // swap 2 and 3 so it makes sense that volume 4 is the BR but so the arrow
    // goes in a circle
...
void OverviewPage::tick() {
    ...
    if (btn->gotDoubleClicked()) volumes[arrowPos]->toggleMute();
    if (enc->pulseRight) {
        if (!selected)
...
void OverviewPage::drawPage() {
    for (int i = 0; i < 4; i++) {
        ...
        if (arrowPos == i) arrowPrinter(selected xor leftFacing);
    }
}

void OverviewPage::volChangePrint(VolumeTracker *tracker, bool isInput, byte x,
                                  byte y, bool leftFacing) {
