 ...
 * but only for muting or changing volume other functions have to be done on the
 * volumes main page
 */
class OverviewPage : public Page {
   private:
    ...
    // extra lcd functions
    void volChangePrint(VolumeTracker *tracker, bool isInput, byte x, byte y,
                        bool leftFacing);
   ...
   public:
    OverviewPage(String name, LiquidCrystal *display, Button *btn, Encoder *enc,
                 VolumeTracker *volumeTrackers[], bool areInput[]);
    ...
    VolumeTracker **getVolumeTrackers() { return volumes; }
