...
Button::Button(byte pin)
    : pin(pin), changed(false), lastChangeTime(0), state(digitalRead(pin)) {
...
unsigned Button::getTime() { return millis() - lastChangeTime; }
    ...
    if (gotPressed) {
        if (waitingForSecondPress && (millis() - firstPressTime <= time)) {
            waitingForSecondPress =
                false;    // Reset after detecting double-click
            return true;  // Double-click detected
        }

        // Record the time of the first press and start waiting for the second
        // press
...
void Button::setDebounceDelay(unsigned delay) { debounceDelay = delay; }